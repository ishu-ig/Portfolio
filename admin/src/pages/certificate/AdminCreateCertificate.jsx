...
import { createCertificate, getCertificate } from "../../Redux/ActionCreartors/CertificateActionCreators"
...
export default function AdminCreateCertificate() {
    ...
    let [error, setError] = useState({
        ...
        issuedBy: "Issued Field is Mendatory"
    })
            ...
            <div className="container">
                ...
                <div className="card mt-3 shadow-sm p-4">
                    <form onSubmit={postSubmit}>
                        <div className="mb-3">
                            <label>Name*</label>
                            <input type="text" name="name" onChange={getInputData} placeholder='Certificate Name' className={`form-control border-3 ${show && error.name ? 'border-danger' : 'border-primary'}`} />
                            {show && error.name ? <p className='text-danger text-capitalize'>{error.name}</p> : null}
                        </div>
                        <div className="mb-3">
                            <label>Issued BY*</label>
                            <input type="text" name="issuedBy" onChange={getInputData} placeholder='Certificate Name' className={`form-control border-3 ${show && error.issuedBy ? 'border-danger' : 'border-primary'}`} />
                            {show && error.issuedBy ? <p className='text-danger text-capitalize'>{error.issuedBy}</p> : null}
                        </div>
